...
KeyCode joystick_get_key_loop( void ) {
  ...
  if ( !( PORT_ReadInputDataBit( MDR_PORTC, PORT_Pin_2 ) ) ) _js_data[0] = ( _js_data[0] |= 0x1UL ); /* SEL      PC2*/
  if ( !( PORT_ReadInputDataBit( MDR_PORTB, PORT_Pin_6 ) ) ) _js_data[1] = ( _js_data[1] |= 0x1UL ); /* RIGHT    PB6*/
  if ( !( PORT_ReadInputDataBit( MDR_PORTE, PORT_Pin_3 ) ) ) _js_data[2] = ( _js_data[2] |= 0x1UL ); /* LEFT     PE3*/
  if ( !( PORT_ReadInputDataBit( MDR_PORTB, PORT_Pin_5 ) ) ) _js_data[3] = ( _js_data[3] |= 0x1UL ); /* UP       PB5*/
  if ( !( PORT_ReadInputDataBit( MDR_PORTE, PORT_Pin_1 ) ) ) _js_data[4] = ( _js_data[4] |= 0x1UL ); /* DOWN     PE1*/
  ...
  if ( sKey == 0 )
    ...
    for ( i = 0; i < 5; i++ )
      if ( actual_data[i] == 1 ) return ( (KeyCode) ( i + 1 ) );  // Если нажата только одна кнопка, то распознаем её
  }
  return NOKEY;
}
